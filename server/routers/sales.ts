...
export const salesRouter = router({
  ...
  cancel: managementProcedure.input(z.object({ saleId: z.number().int().positive(), reason: z.string().trim().min(3).max(255) })).mutation(({ ctx, input }) => cancelSale(input.saleId, ctx.user.id, input.reason)),
  createReturn: managementProcedure.input(z.object({ saleId: z.number().int().positive(), reason: z.string().trim().min(3).max(255), refundMethod: z.enum(paymentMethods), items: z.array(z.object({ saleItemId: z.number().int().positive(), quantity: z.coerce.number().positive() })).min(1) })).mutation(({ ctx, input }) => createSaleReturn({ ...input, userId: ctx.user.id })),
