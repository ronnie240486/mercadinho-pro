...
import { isFractionalProductUnit, normalizeSaleQuantity } from "@/lib/productUnits";
import { loadOperationSettings } from "@/lib/operationSettings";
import { printThermalReceipt, type ThermalReceipt } from "@/lib/thermalReceipt";
import { trpc } from "@/lib/trpc";
import { Banknote, Barcode, CreditCard, Loader2, Minus, PackageSearch, Plus, Printer, Search, ShieldCheck, ShoppingBag, Trash2, UserRoundPlus } from "lucide-react";
...
export default function Pdv() {
  ...
  const [lastReceipt, setLastReceipt] = useState<ThermalReceipt | null>(null);
  ...
  const availableLoyaltyDiscount = selectedCustomer?.loyaltyMode === "points" ? selectedCustomer.loyaltyPointsBalance / 100 : toNumber(selectedCustomer?.loyaltyCreditBalance);
  const requestedLoyaltyUse = Math.max(Number(loyaltyUse) || 0, 0);
  const requestedLoyaltyDiscount = selectedCustomer?.loyaltyMode === "points" ? Math.floor(requestedLoyaltyUse) / 100 : requestedLoyaltyUse;
  const safeLoyaltyDiscount = Math.min(requestedLoyaltyDiscount, availableLoyaltyDiscount || 0, Math.max(0, subtotal - safeDiscount));
  const loyaltyPointsUsed = selectedCustomer?.loyaltyMode === "points" ? Math.round(safeLoyaltyDiscount * 100) : 0;
  const total = Math.round((subtotal - safeDiscount - safeLoyaltyDiscount) * 100) / 100;
  const createSale = trpc.sales.create.useMutation({
    onSuccess: async result => {
      const paymentLabel = paymentOptions.find(option => option.value === paymentMethod)?.label ?? "Pagamento";
      setLastReceipt({ code: result.code, issuedAt: new Date(), paymentLabel, subtotal, discount: safeDiscount + safeLoyaltyDiscount, total: result.totalAmount, items: cart.map(item => ({ name: item.name, quantity: item.quantity, unit: item.unit, unitPrice: item.salePrice, total: item.salePrice * item.quantity })) });
      ...
      setCart([]); setSearch(""); setCustomerId(""); setDiscount("0"); setLoyaltyUse("0");
  ...
  const commitQuantity = (productId: number, rawValue: string) => {
    setCart(current => current.map(item => {
      if (item.id !== productId) return item;
      try {
        const next = normalizeSaleQuantity(rawValue, item.unit);
    ...
    setQuantityDrafts(current => { const next = { ...current }; delete next[productId]; return next; });
  };
  ...
  const finalizeSale = () => {
    ...
    createSale.mutate({ customerId: customerId ? Number(customerId) : null, discountAmount: safeDiscount, loyaltyRedemption: selectedCustomer?.loyaltyMode === "points" ? { points: loyaltyPointsUsed } : selectedCustomer ? { creditAmount: safeLoyaltyDiscount } : undefined, items: cart.map(item => ({ productId: item.id, quantity: item.quantity })), payments: [{ method: paymentMethod, amount: total }] });
  };

  const printLastReceipt = () => {
    if (!lastReceipt) return;
    try {
      printThermalReceipt(lastReceipt, loadOperationSettings().receiptWidth);
    } catch (error) {
  ...
  return <div className="mx-auto max-w-[1480px]"><PageHeader eyebrow="Operação" title="Frente de caixa" description="Registre uma nova venda, encontre itens por nome ou código de barras e conclua pelo meio de pagamento adequado." action={cashLoading ? <span className="flex items-center gap-2 rounded-lg bg-slate-200 px-3 py-2 text-xs font-bold text-slate-700"><Loader2 className="h-3.5 w-3.5 animate-spin" />Verificando caixa</span> : <span className={`rounded-lg px-3 py-2 text-xs font-bold ${cashSession ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{cashSession ? "Caixa aberto" : "Caixa fechado"}</span>} />
    ...
    <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#eadfbf] bg-[#fff8e8] px-4 py-3 text-sm text-[#6f5311]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#a36c00]" /><p><strong>Controle de operação:</strong> vendas são realizadas normalmente no PDV. Sangrias, ajustes, cancelamentos e devoluções exigem acesso de gerente ou administrador.</p></div>
    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_390px]"><article className="rounded-2xl border border-white/80 bg-white p-4 shadow-[0_10px_28px_rgba(37,59,48,0.06)] sm:p-6"><div className="flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-emerald-800/60" /><Input autoFocus value={search} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); scanCurrentInput(); } }} onChange={event => setSearch(event.target.value)} placeholder="Escaneie ou busque por nome e código" className="h-14 rounded-xl border-[#cfd9d1] bg-[#fbfcfa] pl-12 pr-12 text-base shadow-none placeholder:text-slate-400 focus-visible:ring-emerald-700" /><Barcode className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /></div><BarcodeScannerButton onDetected={scanFromCamera} label="Câmera" /></div><p className="mt-2 text-xs text-slate-500">Leitor USB/Bluetooth: deixe este campo ativo, escaneie o código e pressione Enter automaticamente.</p>
      ...
      <div className="mt-5 flex min-h-[370px] flex-col rounded-2xl border border-dashed border-[#cbd8cd] bg-[#fafcf9] px-3 py-4 sm:px-5">{cart.length ? <div className="space-y-2">{cart.map(item => { const allowsFraction = isFractionalProductUnit(item.unit); return <div key={item.id} className="flex items-center gap-3 rounded-xl bg-white p-3 shadow-sm"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-xs font-bold text-emerald-800">{item.name.slice(0, 1).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-[#294239]">{item.name}</p><p className="mt-0.5 text-xs text-slate-500">{formatCurrency(item.salePrice)} por {item.unit}{allowsFraction ? " · informe até 3 casas decimais" : " · quantidade inteira"}</p></div><div className="flex items-center gap-1 rounded-lg border border-[#dce5dd] p-0.5"><Button onClick={() => { setQuantityDrafts(current => { const next = { ...current }; delete next[item.id]; return next; }); updateQuantity(item.id, -1); }} variant="ghost" size="icon" className="h-7 w-7 rounded-md text-slate-600 hover:bg-[#f0f5f0]"><Minus className="h-3.5 w-3.5" /></Button><Input value={quantityDrafts[item.id] ?? String(item.quantity)} inputMode="decimal" onChange={event => setQuantityDrafts(current => ({ ...current, [item.id]: event.target.value }))} onKeyDown={event => { if (event.key === "Enter") event.currentTarget.blur(); }} onBlur={event => commitQuantity(item.id, event.target.value)} aria-label={`Quantidade de ${item.name}`} className="h-7 w-14 border-0 bg-transparent px-0 text-center text-sm font-semibold text-[#294239] shadow-none focus-visible:ring-0" /><Button onClick={() => { setQuantityDrafts(current => { const next = { ...current }; delete next[item.id]; return next; }); updateQuantity(item.id, 1); }} variant="ghost" size="icon" className="h-7 w-7 rounded-md text-slate-600 hover:bg-[#f0f5f0]"><Plus className="h-3.5 w-3.5" /></Button></div><p className="w-20 text-right text-sm font-bold text-[#17332c]">{formatCurrency(item.salePrice * item.quantity)}</p></div>; })}</div> : <div className="m-auto flex flex-col items-center justify-center px-5 text-center"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e6f3e9] text-emerald-800"><PackageSearch className="h-6 w-6" /></span><h2 className="mt-5 font-serif text-xl font-semibold text-[#17332c]">O carrinho está aguardando itens.</h2><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">Pesquise um produto acima ou utilize um leitor de código de barras para adicioná-lo à venda.</p></div>}</div>
    </article><aside className="rounded-2xl border border-[#dce6dd] bg-[#173f32] p-5 text-white shadow-[0_16px_36px_rgba(23,63,50,0.18)] sm:p-6"><div className="flex items-start justify-between gap-3 border-b border-white/10 pb-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-100/60">Venda atual</p><h2 className="mt-1 font-serif text-xl font-semibold">Novo atendimento</h2></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10"><ShoppingBag className="h-5 w-5" /></span></div><label className="mt-5 block"><span className="mb-1.5 flex items-center gap-2 text-xs font-semibold text-emerald-50/80"><UserRoundPlus className="h-4 w-4" />Cliente</span><select value={customerId} onChange={event => { setCustomerId(event.target.value); setLoyaltyUse("0"); }} className="h-11 w-full rounded-xl border border-white/15 bg-white/5 px-3 text-sm text-white outline-none"><option className="text-slate-900" value="">Consumidor final</option>{customers?.map(customer => <option className="text-slate-900" key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label>{selectedCustomer ? <label className="mt-4 block rounded-xl border border-[#f3bd32]/35 bg-[#f3bd32]/10 p-3"><span className="flex items-center justify-between gap-2 text-xs font-semibold text-[#fff0c5]"><span>Fidelidade: {selectedCustomer.loyaltyMode === "points" ? "pontos" : "crédito"}</span><span>{selectedCustomer.loyaltyMode === "points" ? `${selectedCustomer.loyaltyPointsBalance} pts` : formatCurrency(selectedCustomer.loyaltyCreditBalance)}</span></span><Input value={loyaltyUse} type="number" min="0" max={selectedCustomer.loyaltyMode === "points" ? selectedCustomer.loyaltyPointsBalance : availableLoyaltyDiscount} step={selectedCustomer.loyaltyMode === "points" ? "1" : "0.01"} onChange={event => setLoyaltyUse(event.target.value)} placeholder={selectedCustomer.loyaltyMode === "points" ? "Pontos a usar" : "Crédito a usar"} className="mt-2 h-10 border-white/15 bg-white/5 text-white placeholder:text-emerald-50/40" /><span className="mt-1.5 block text-[11px] text-emerald-50/60">{selectedCustomer.loyaltyMode === "points" ? "100 pontos equivalem a R$ 1,00 de desconto." : "O crédito será abatido diretamente do total."}</span></label> : null}<label className="mt-4 block"><span className="mb-1.5 block text-xs font-semibold text-emerald-50/80">Desconto manual (R$)</span><Input value={discount} type="number" min="0" max={subtotal} step="0.01" onChange={event => setDiscount(event.target.value)} className="h-11 border-white/15 bg-white/5 text-white placeholder:text-emerald-50/40" /></label><div className="mt-6 space-y-3 border-t border-white/10 pt-5 text-sm"><div className="flex justify-between text-emerald-50/70"><span>Subtotal</span><span>{formatCurrency(subtotal)}</span></div><div className="flex justify-between text-emerald-50/70"><span>Desconto manual</span><span>- {formatCurrency(safeDiscount)}</span></div>{selectedCustomer ? <div className="flex justify-between text-emerald-50/70"><span>Fidelidade</span><span>- {formatCurrency(safeLoyaltyDiscount)}</span></div> : null}<div className="flex items-end justify-between pt-2"><span className="font-medium text-emerald-50/80">Total</span><strong className="font-serif text-3xl tracking-tight">{formatCurrency(total)}</strong></div></div><div className="mt-6 grid grid-cols-2 gap-2">{paymentOptions.map(option => <Button key={option.value} onClick={() => setPaymentMethod(option.value)} variant="outline" className={`h-11 rounded-xl border-white/15 text-xs hover:text-white ${paymentMethod === option.value ? "bg-[#d8f0df] text-[#164e3d] hover:bg-[#d8f0df]" : "bg-white/5 text-white hover:bg-white/10"}`}>{option.value === "cash" ? <Banknote className="mr-1 h-4 w-4" /> : option.value === "debit" || option.value === "credit" ? <CreditCard className="mr-1 h-4 w-4" /> : null}{option.label}</Button>)}</div><Button onClick={finalizeSale} disabled={!cart.length || !cashSession || createSale.isPending} className="mt-5 h-12 w-full rounded-xl bg-[#d8f0df] text-sm font-bold text-[#164e3d] hover:bg-[#c7e8d0] disabled:bg-white/20 disabled:text-white/40">{createSale.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ShoppingBag className="mr-2 h-4 w-4" />}Finalizar venda</Button>{lastReceipt ? <Button onClick={printLastReceipt} variant="outline" className="mt-2 h-10 w-full rounded-xl border-[#f3bd32]/35 bg-white/5 text-[#fff0c5] hover:bg-white/10 hover:text-white"><Printer className="mr-2 h-4 w-4" />Imprimir último comprovante</Button> : null}<Button onClick={() => setCart([])} disabled={!cart.length} variant="ghost" className="mt-2 h-10 w-full rounded-xl text-emerald-50/70 hover:bg-white/5 hover:text-white"><Trash2 className="mr-2 h-4 w-4" />Cancelar atendimento</Button></aside></section>
  </div>;
}
