...
import { productCsvTemplate, productCsvTemplateFilename } from "@/lib/productCsvTemplate";
import { formatCurrency } from "@/lib/format";
import { trpc } from "@/lib/trpc";
import JsBarcode from "jsbarcode";
import { CheckSquare, Download, FileSpreadsheet, FileUp, Loader2, Printer, ScanBarcode, Square } from "lucide-react";
...
export default function Utilities() {
  ...
  const downloadProductTemplate = () => {
    const file = new Blob([productCsvTemplate], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(file);
    const link = document.createElement("a");
    link.href = url;
    link.download = productCsvTemplateFilename;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    toast.success("Modelo CSV baixado. Preencha uma linha para cada produto e importe quando terminar.");
  };
  ...
  return <div className="mx-auto max-w-[1200px]">
    ...
    <section className="grid gap-6 xl:grid-cols-2">
      <article className="rounded-2xl border border-white/80 bg-white p-5 shadow-[0_10px_28px_rgba(16,24,32,0.06)] sm:p-6">
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#fff0c5] text-[#a36c00]"><FileSpreadsheet className="h-5 w-5" /></span>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-serif text-xl font-semibold text-[#101820]">Importar catálogo CSV</h2><p className="mt-2 text-sm leading-6 text-slate-500">Envie um arquivo ou cole os dados. A primeira linha pode usar os campos <strong>Nome; Código de barras; Código interno; Categoria; Unidade; Custo; Preço de venda; Estoque mínimo</strong>. Também aceitamos o formato simples: nome; código; venda; mínimo.</p></div><Button type="button" variant="outline" onClick={downloadProductTemplate} className="h-10 shrink-0 rounded-xl border-[#d3b25a] text-[#8a6207] hover:bg-[#fff6da]"><Download className="mr-2 h-4 w-4" />Baixar modelo</Button></div>
        <div className="mt-4 rounded-xl border border-[#eadfbf] bg-[#fffaf0] p-3 text-xs leading-5 text-[#6f5311]"><strong>Como preencher:</strong> uma linha por produto; use siglas como <strong>UN, PCT, CX, FD, KG, G, L ou ML</strong>; deixe código de barras e código interno vazios apenas se o produto não tiver identificação. O modelo baixado contém somente o cabeçalho, sem dados de exemplo para evitar importação acidental.</div>
        <label className="mt-5 flex h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-[#d3b25a] bg-[#fffaf0] px-4 text-sm font-semibold text-[#a36c00] hover:bg-[#fff0c5]"><FileUp className="h-4 w-4" />Selecionar arquivo CSV<Input type="file" accept=".csv,text/csv,text/plain" onChange={loadFile} className="sr-only" /></label>
        <textarea value={csv} onChange={event => updatePreview(event.target.value)} placeholder={"Nome;Código de barras;Categoria;Unidade;Custo;Preço de venda;Estoque mínimo\nArroz tipo 1;7891234567890;Mercearia;PCT;18,50;24,90;5"} className="mt-4 min-h-32 w-full rounded-xl border border-[#d9dfe5] bg-[#fbfcfd] p-3 text-sm outline-none focus:border-[#e3aa14] focus:ring-2 focus:ring-[#e3aa14]/20" />
        {preview ? <div className="mt-4 rounded-xl border border-[#e0e5ea] p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-[#101820]">Prévia: {preview.items.length} produto(s) válido(s)</p><span className="text-xs text-slate-500">{preview.detectedHeader ? "Cabeçalho identificado" : "Formato simples"}</span></div>{preview.items.length ? <div className="mt-3 max-h-36 space-y-1 overflow-y-auto text-xs text-slate-600">{preview.items.slice(0, 8).map((item, index) => <p key={`${item.name}-${index}`}>{item.name} · {item.unit} · {formatCurrency(item.salePrice)} · mínimo {item.minimumStock}</p>)}{preview.items.length > 8 ? <p>+ {preview.items.length - 8} produto(s) na prévia</p> : null}</div> : null}{preview.errors.length ? <div className="mt-3 rounded-lg bg-rose-50 p-3 text-xs text-rose-800">{preview.errors.slice(0, 4).map(error => <p key={error}>{error}</p>)}{preview.errors.length > 4 ? <p>+ {preview.errors.length - 4} erro(s)</p> : null}</div> : null}<Button onClick={() => importProducts.mutate({ items: preview.items })} disabled={!preview.items.length || preview.errors.length > 0 || importProducts.isPending} className="mt-4 h-11 w-full rounded-xl bg-[#e3aa14] text-[#101820] hover:bg-[#f5c243]">{importProducts.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Importar {preview.items.length} produto(s)</Button></div> : null}
      </article>
